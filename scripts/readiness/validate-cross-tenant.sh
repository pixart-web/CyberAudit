...
tests=(
  ...
  apps/api/tests/test_agent_runtime.py::test_tool_gateway_get_finding_is_tenant_scoped
  apps/api/tests/test_agent_runtime.py::test_tool_gateway_list_incidents_only_returns_the_caller_tenant
  apps/api/tests/test_engagement_services.py::test_record_timeline_event_is_tenant_scoped
  apps/api/tests/test_security_hardening.py::test_report_generation_cannot_be_triggered_across_tenants
  apps/api/tests/test_security_hardening.py::test_agent_context_never_crosses_tenants_even_with_matching_source_ids
)
...
jq -n --arg executed_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  ...
  '{status:"passed",executed_at:$executed_at,test_count:$count,
    layers:["policy","orchestrator","graph","connector","worker","evidence","dlq","execution_api","ai_agents","engagement_reporting"],
