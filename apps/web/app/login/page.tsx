...
const schema = z.object({ email: z.string().email("Introduza um email válido."), password: z.string().min(8, "A password deve ter pelo menos 8 caracteres.") });
type FormData = z.infer<typeof schema>;

export default function LoginPage() {
  const router = useRouter();
  const [error, setError] = useState("");
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { email: "admin@cyberaudit.local", password: "ChangeMe123!" } });
  const submit = async (values: FormData) => {
    setError("");
    try {
      const token = await api<{ access_token: string; refresh_token: string }>("/auth/login", { method: "POST", body: JSON.stringify(values) });
      sessionStorage.setItem("access_token", token.access_token);
      sessionStorage.setItem("refresh_token", token.refresh_token);
      router.push("/dashboard");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Falha na autenticação."); }
  };
  return <main className="relative grid min-h-screen place-items-center overflow-hidden px-4">
