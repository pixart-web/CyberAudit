...
export function ResourcePage({ title, endpoint, createHref, columns }: { title: string; endpoint: string; createHref?: string; columns: [string, string][] }) {
  ...
  return <Shell title={title}>
    <Card className="overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="table"><thead><tr>{columns.map(([key,label])=><th key={key}>{label}</th>)}</tr></thead><tbody>
        ...
        {data?.items.map((item,index)=><tr key={String(item.id ?? index)}>{columns.map(([key])=><td key={key}>{key === "status" || key === "result" ? <Badge tone={["active","success","valid"].includes(String(item[key]))?"success":"warning"}>{String(item[key] ?? "—")}</Badge> : key === "created_at" ? new Intl.DateTimeFormat("pt-PT",{dateStyle:"medium",timeStyle:"short"}).format(new Date(String(item[key]))) : String(item[key] ?? "—")}</td>)}</tr>)}
